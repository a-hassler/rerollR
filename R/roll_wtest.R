#'Performs Wilcoxon-Mann-Whitney tests (Wilcoxon rank sum tests) on data frames
...
#----------------------
#Core code
#Wilcox test between col of p1 and p2
wilres <-  data.frame(last=1:1) #create the df to store wilcox results
k=1
repeat{
  wilres[,"last"]<- NA
  wilres[,ncol(wilres)]<-wilcox.test(x=p1[,k],y=p2[,k], ...)$p.value
  names(wilres)[ncol(wilres)] <- k
  k=k+1
  if(ncol(p1)<k){break}
}
...
res_list<- data.frame("p0.001"= length(wilres[,wilres <= 0.001])/length(wilres)*100, "p0.01"= length(wilres[,wilres <= 0.01])/length(wilres)*100, "p0.05"= length(wilres[,wilres <= 0.05])/length(wilres)*100)# create a list of statistics about the tests
twilres <- data.frame(p.value=t(wilres))

#----------------------
#Plot results
if (do.plot==T) {

  d <- with(density(t(wilres), from=0), data.frame(x, y))
  #conditional density parameters to improve readability for low p values and prevent density curve to start at x<0
  if(max(d$x)<0.001 & min(d$x)<0) {d <- with(density(twilres$p.value, from=0, to=0.001), data.frame(x, y))}
  if(min(d$x)<0) {d <- with(density(twilres$p.value, from=0), data.frame(x, y))}
  if(max(d$x)<0.001) {d <- with(density(twilres$p.value, to=0.001), data.frame(x, y))}

  #scale density to the max of the histogram if combined density + histogram plot
  if(histogram==T) d[,"y"]<- d[,"y"]/max(d$y)*100*max(proportions(table(trunc(twilres$p.value*10^3)/10^3)))
  ...
  resplot<- ggplot2::ggplot(data = d, mapping = ggplot2::aes(x = x, y = y)) +
    ...
    {if(density==T && histogram == T)ggplot2::geom_histogram(data = twilres, ggplot2::aes(x=p.value, ggplot2::after_stat(count/sum(count)*100)), boundary=0, binwidth = 0.001, color="grey50", fill="white", alpha=0.1, na.rm = T)} +
    ...
    {if(all(twilres$p.value <= 0.001)) ggplot2::annotate("text", x = 0.025, y= max(d$y)/2, label= (paste("All p-values are below 0.001")), cex = 4)}+

    #Axis and theme
    {if(density==T)ggplot2::scale_x_continuous(limits=c(0, ifelse(max(d$x) < 0.05, 0.05, max(d$x))))}+
    {if(density==F)ggplot2::scale_x_continuous(limits=c(0, ifelse(max(twilres) < 0.05, 0.05, max(twilres))))}+
    ggplot2::theme_bw()+
    {if(density==T && histogram==F)ggplot2::labs(title= paste("Wilcoxon-Mann-Whitney tests (",deparse(substitute(p1)), " Vs ", deparse(substitute(p2)),")"), y="Density", x= "p-value")}+
    {if(histogram==T)ggplot2::labs(title= paste("Wilcoxon-Mann-Whitney tests (",deparse(substitute(p1)), " Vs ", deparse(substitute(p2)),")"), y="Percentage (%)", x= "p-value")}

} #plot end

#----------------------
#Return results
ifelse(do.plot==T, all_res<-list(pvalues= twilres, stats=res_list, plot = resplot), all_res<-list(pvalues= twilres, stats=res_list, "plot" = "No plot generated. do.plot set to F."))
return(all_res)
